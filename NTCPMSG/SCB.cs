...
namespace NTCPMSG
{
    ...
    class SCB
    {
        ...
        internal void AsyncSend(byte[] buf, int offset, int size)
        {
            ...
            IAsyncResult m_result = WorkSocket.BeginSend(buf, offset, size, SocketFlags.None,
                  new AsyncCallback(Async_Send), WorkSocket);
            System.Threading.ThreadPool.RegisterWaitForSingleObject(m_result.AsyncWaitHandle, WaitOrTimerCallback,
                null, 3 * 1000, false);
        ...
        internal void AsyncSend(byte[] buf)
        {
            ...
            IAsyncResult m_result = WorkSocket.BeginSend(buf, 0, buf.Length, SocketFlags.None,
                new AsyncCallback(Async_Send), WorkSocket);
            System.Threading.ThreadPool.RegisterWaitForSingleObject(m_result.AsyncWaitHandle, WaitOrTimerCallback,
               null, 3 * 1000, false);
